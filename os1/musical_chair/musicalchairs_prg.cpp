 ...
 * Author:  J.Sai Nishith , P.V.Asish
 * Roll# :  CS18BTECH11018, CS18BTECH11037
 */
	  ...
	  if (temp == "lap_start")
	    {
	      ...
	      count = 0; //setting number of chairs occupied to 0
	    
	    }
	  if (temp == "music_start")
	    {
	      cv.notify_all (); //synchronising all the player threads
	    }
	  if (temp == "umpire_sleep")
	    {
	      int timeOfSleep;
	      cin >> timeOfSleep;
	      this_thread::sleep_for (chrono::microseconds (timeOfSleep)); //making umpire sleep
	    }
	  ...
	  if (temp == "music_stop")
	    {
	      ...
	      play = 1;
	      cv2.wait (wait_lock1); //waiting for signal from player who lost
	    }
	  if (temp == "lap_stop")
	    {
	      lap++; //increasing the lap count
	      n--; //decreasing player count
	      if (lap == nplayers)
		{
		  cv.notify_one (); //giving signal to the winner
		}
	      play = 0; 
	    }
	}
...
player_main (int plid)
{
  while (1)
    {
      if (playersleep[plid] != 0) //checking for player sleep condition
	{
	  this_thread::sleep_for (chrono::microseconds (playersleep[plid]));
	  playersleep[plid] = 0;
	}

      if (n == 1) //winner thread
	{
	  cout << "Winner is " << plid << "\n";
	  break;
	}

      if (play) //when players allowed to aquire chairs
	{
	  std::unique_lock < std::mutex > wait_lock (m);

	  if (count == n - 1) //player who lost
	    {
	      cout << "======= lap# " << lap << " =======\n";
	  ...
	  count++; //player incrementing number of chairs occupied 

	  cv.wait (wait_lock); //players waiting for signal from umpire
...
musical_chairs (int nplayers)
{
  ...
  cout << "Musical Chairs: " << nplayers << " player game with " << nplayers -
    1 << " laps.\n";
  thread umpire (umpire_main, nplayers); //spawn umpire thread
  for (int i = 0; i < nplayers; i++)
    {
      players[i] = thread (player_main, i); //spawn player threads
    }
